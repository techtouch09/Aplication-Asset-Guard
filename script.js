...
function cariBarang() {
    ...
    const hasil = kata ? daftarBarang.filter(b => 
        ...
        (b.lokasi && b.lokasi.toLowerCase().includes(kata)) // Tambah pencarian lokasi
    ) : daftarBarang;
...
function tampilkanPeminjaman() {
    ...
    daftarPinjam.forEach((item, i) => {
        ...
        baris.innerHTML = `
            ...
            <td>${item.shift}</td>
            <td>
                ${status === "🔄 Belum Selesai" ? `<button class="btn btn-edit" onclick="bukaEditPinjam(${item.id})">Edit</button>` : ""}
...
function tampilkanPengembalian() {
    ...
    daftarKembali.forEach((item, i) => {
        ...
        baris.innerHTML = `
            ...
            <td>${item.shift}</td>
            <td>
                <button class="btn btn-edit" onclick="bukaEditKembali(${item.id})">Edit</button>
...
function cetakPerItem(jenis) {
    ...
    if (jenis === "barang") {
        ...
        kolom = ["NO", "TANGGAL", "KODE", "PEKERJA", "NAMA BARANG", "JUMLAH AWAL", "DIPINJAM", "SISA STOK", "SHIFT", "LOKASI"];
        data = data.map((b,i) => ({
            no:i+1,
        ...
        kolom = ["NO", "TANGGAL", "KODE", "NAMA BARANG", "PEMINJAM", "DIPINJAM", "SUDAH KEMBALI", "SHIFT"];
        data = data.map((p,i) => ({
            no:i+1,
        ...
        kolom = ["NO", "TANGGAL PINJAM", "TANGGAL KEMBALI", "KODE", "NAMA BARANG", "PEMINJAM", "DIKEMBALIKAN", "SHIFT"];
        data = data.map((k,i) => {
...
function cetakPDFKeseluruhan(data, mulai, selesai) {
    ...
    if (data.barang.length) {
        ...
        doc.autoTable({
            startY:y,
            head:[["NO","TANGGAL","KODE","PEKERJA","NAMA","JUMLAH","DIPINJAM","SISA STOK","SHIFT","LOKASI"]],
    ...
    if (data.pinjam.length) {
        ...
        doc.autoTable({
            startY:y,
            head:[["NO","TANGGAL","KODE","NAMA","PEMINJAM","DIPINJAM","SUDAH KEMBALI","SHIFT"]],
            body:data.pinjam.map((p,i) => [
                i+1, new Date(p.tanggal).toLocaleDateString("id-ID"), p.kode, p.nama, p.pekerja, 
    ...
    if (data.kembali.length) {
        ...
        doc.autoTable({
            startY:y,
            head:[["NO","TANGGAL PINJAM","TANGGAL KEMBALI","KODE","NAMA","PEMINJAM","DIKEMBALIKAN","SHIFT"]],
...
function cetakExcelPerItem(jenis) {
    ...
    if (jenis === "barang") {
        judul = "LOG BARANG MASUK";
        header = ["NO", "TANGGAL", "KODE", "PEKERJA", "NAMA", "JUMLAH", "DIPINJAM", "SISA STOK", "SHIFT", "LOKASI"];
        const daftarBarang = JSON.parse(localStorage.getItem("dataBarang")) || [];
    ...
    // ==============================================
// LANJUTAN FUNGSI CETAK EXCEL PER ITEM
// ==============================================
} else if (jenis === "peminjaman") {
    judul = "LOG BARANG KELUAR";
    header = ["NO", "TANGGAL", "KODE", "NAMA", "PEMINJAM", "DIPINJAM", "SUDAH KEMBALI", "SHIFT"];
    const daftarPinjam = JSON.parse(localStorage.getItem("dataPeminjaman")) || [];
    ...
    judul = "LOG BARANG PENGEMBALIAN";
    header = ["NO", "TANGGAL PINJAM", "TANGGAL KEMBALI", "KODE", "NAMA", "PEMINJAM", "DIKEMBALIKAN", "SHIFT"];
    const daftarKembali = JSON.parse(localStorage.getItem("dataPengembalian")) || [];
    const daftarPinjam = JSON.parse(localStorage.getItem("dataPeminjaman")) || [];
